---
title: "Lista 3 - DISTINCT, AND-OR-NOT"
...
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE ORDER BY DNO, SALARY
```
As tabelas são iguais pois não há linhas repetidas nos dados selecionados. Somente a última se difere das demais por está ordenada.
...
5.  Exiba todas as informações sobre qualquer reserva natural cujo valor de ACRES esteja entre 1200 e 15000,  
...
WHERE ACRES >= 50 OR STATE <> 'MA'
```
A primeira e a terceira cláusulas são equivalentes, entretando a segunda não equivale a estas. A primeira e terceira pegam os dados que não são de Massachusetts e nem tenham menos de 50 acres ao mesmo tempo, enquanto a segunda cláusula pega os dados que não tenham menos de 50 acres e nem aqueles que não sejam de Massachusetts, ou seja reservas com menos de 50 acres e sejam de qualquer outro estado não são exibidas.
...
A primeira e a terceira cláusulas são equivalentes, entretando a segunda não equivale a estas.
A primeira e a terceira cláusulas pegam os dados das reservas que não tenham área menor que 50 acres ou sejam de Massachusetts, enquanto a segunda pega os dados que não tenham menos de 50 acres ou não sejam de Massachusetts, assim incluindo aqueles com menos de 50 acres e são de Massachusetts e aqueles que não são de Massachusetts, mas possuem mais de 50 acres.
